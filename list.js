module.exports = function (address){
    ...
    this.validate = async (client) => {
        if (client){
            try {
                const result = await client.query("SELECT * FROM \"public\".\"lists\" WHERE address = $1", [address])
    ...
    this.recipients = async (client) => {
        if (client){
            try {
                const result = await client.query("SELECT A.email FROM \"public\".\"lists\" AS l RIGHT JOIN \"public\".\"listUsers\" AS A ON A.lid=l.id WHERE l.address=$1", [address])
